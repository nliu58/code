""" Functions to do geometric corrections.
...
def build_sca(sca_image_file, imugps_file, igm_image_file, sun_angles, map_crs):
    ...
    sensor_zenith = np.arctan(np.sqrt(DX**2+DY**2)/DZ)
    sensor_azimuth = np.arcsin(np.abs(DX)/np.sqrt(DX**2+DY**2))
    index = (DX>0)&(DY<0)
    sensor_azimuth[index]=np.pi-sensor_azimuth[index]
    index = (DX<0)&(DY<0)
    sensor_azimuth[index]=np.pi+sensor_azimuth[index]
    index = (DX<0)&(DY>0)
    sensor_azimuth[index]=2*np.pi-sensor_azimuth[index]
    del DX, DY, DZ, index
    ...
    sca_image[0,:,:] = np.rad2deg(sensor_zenith)
    sca_image[1,:,:] = np.rad2deg(sensor_azimuth)
    sca_image.flush()
    del sensor_zenith, sensor_azimuth

    # Write scan angle header file
    sca_header = empty_envi_header()
    sca_header['description'] = 'SCA (map coordinate system=%s)' %(map_crs.ExportToWkt())
    ...
    sca_header['band names'] = ['Sensor Zenith', 'Sensor Azimuth']
...
def ray_tracing(XYZ0, XYZ1, V, DEM, DEM_X0Y0, DEM_Resolution):
    ...
    if np.abs(XYZ0[0]-XYZ1[0])<1e-3 and np.abs(XYZ0[1]-XYZ1[1])<1e-3:
    ...
    while 1:
        ...
        if the_x < 0 or the_y < 0 or the_x == x_dim or the_y == y_dim:
